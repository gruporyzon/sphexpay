...
export default async function handler(request,response){
 ...
 if(!configuration.vapidConfigured)return response.status(503).json({success:false,code:'VAPID_NOT_CONFIGURED',message:'As chaves do servidor de notificações não foram configuradas corretamente.'})
 if(!configuration.storageConfigured)return response.status(503).json({success:false,code:'SUPABASE_SERVER_CREDENTIALS_MISSING',message:'As credenciais server-side do armazenamento ainda não foram configuradas.'})
 const supabase=createClient(supabaseUrl(),serviceRoleKey()),token=String(request.headers.authorization||'').replace(/^Bearer\s+/i,'').trim()
 ...
 try{webpush.setVapidDetails(configuration.vapid.subject,configuration.vapid.publicKey,configuration.vapid.privateKey)}catch{return response.status(503).json({success:false,code:'VAPID_NOT_CONFIGURED',message:'As chaves do servidor de notificações não foram configuradas corretamente.'})}
