...
const allowedModeTypes=new Set(['sale_approved','pix_paid','credit_card_approved','boleto_paid','subscription_approved'])
...
export default async function handler(request,response){
 ...
 const generator=input.type==='generator_notification',manual=input.type==='manual_notification',mode=input.type==='mode_notification'
 if(input.type!=='infrastructure_test'&&!generator&&!manual&&!mode)return response.status(400).json({success:false,code:'INVALID_PAYLOAD',message:'Tipo de notificação inválido.'})
 const eventId=clean(input.eventId)
 const prefix=input.type==='infrastructure_test'?'infrastructure-test-':mode?'mode-sale:':manual?'manual-':'generator-'
