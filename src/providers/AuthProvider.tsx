...
export function AuthProvider({children}:PropsWithChildren){
 ...
 const value=useMemo<AuthContextValue>(()=>({session,user:session?.user??null,loading,configured:isSupabaseClientAvailable,signOut:async()=>{try{await authService.signOut()}finally{clearAuthEntranceState();setSession(null)}}}),[session,loading])
