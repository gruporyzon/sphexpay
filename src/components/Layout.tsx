...
import { ArrowLeftRight,Bell,ChevronDown,Crown,FileBarChart,Landmark,LayoutDashboard,Link2,LogOut,Menu,MessageCircle,Moon,Package,PanelLeftClose,PanelLeftOpen,PanelTop,PlugZap,RadioTower,RefreshCcw,Settings,ShoppingBag,Store,Sun,Trophy,X,type LucideIcon } from 'lucide-react'
...
const groups:readonly NavGroup[]=[
 ...
 {label:'Operação',items:[['Vendas','/app/vendas',ShoppingBag],['Transações','/app/transacoes',ArrowLeftRight],['Produtos','/app/produtos',Package],['Vitrine','/app/vitrine',Store],['Assinaturas','/app/assinaturas',RefreshCcw]]},
...
export function Layout(){
 ...
 const title=loc.pathname.startsWith('/app/financeiro')?'Financeiro':loc.pathname.startsWith('/app/relatorios')?'Relatórios':items.find(item=>item[1]===loc.pathname)?.[0]||'SphexPay',name=user?.user_metadata?.full_name||user?.email?.split('@')[0]||'Conta SphexPay'
