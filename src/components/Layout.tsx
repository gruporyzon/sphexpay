...
import { Bell,Crown,FileBarChart,Landmark,LayoutDashboard,LogOut,Menu,MessageCircle,Package,PanelLeftClose,PanelLeftOpen,PlugZap,RadioTower,RefreshCcw,ShoppingBag,Store,Trophy,UserRoundCheck,X,type LucideIcon } from 'lucide-react'
...
const groups:readonly NavGroup[]=[
 ...
 {label:'Crescimento',items:[['Social','/app/social',MessageCircle],['Afiliados','/app/afiliados',UserRoundCheck],['Premiações','/app/premiacoes',Trophy],['Relatórios','/app/relatorios',FileBarChart],['Notificações','/app/notificacoes',Bell]]},
] as const
...
export function Layout(){
 ...
 const title=loc.pathname.startsWith('/app/produtos')?'Produtos':loc.pathname.startsWith('/app/vendas')?'Vendas':loc.pathname.startsWith('/app/financeiro')?'Financeiro':loc.pathname.startsWith('/app/relatorios')?'Relatórios':loc.pathname.startsWith('/app/configuracoes')?'Configurações':items.find(item=>item[1]===loc.pathname)?.[0]||'SphexPay',name=user?.user_metadata?.full_name||user?.email?.split('@')[0]||'Conta SphexPay'
