...
export function Layout(){
 ...
 return <div className={cn('min-h-screen app-shell internal-app-shell',collapsed?'sidebar-collapsed':'sidebar-expanded')}>
  <aside id="app-navigation" aria-label="Menu principal" role={mobile?'dialog':undefined} aria-modal={mobile||undefined} className={cn('app-sidebar internal-sidebar fixed inset-y-0 left-0 z-50',collapsed?'w-[84px]':'w-[268px]',mobile&&'mobile-open')}>
   <div className="sidebar-brand"><SphexPayLogo showName={!collapsed||mobile} priority adaptiveTheme/><span className="sidebar-brand-signal" aria-hidden="true"><i/><i/></span><button ref={closeButton} onClick={()=>setMobile(false)} className="sidebar-mobile-close mobile-sidebar-close" aria-label="Fechar menu"><X/></button></div>
  ...
  {mobile&&<button className="app-menu-backdrop" aria-label="Fechar menu" onClick={()=>setMobile(false)}/>}<div className="app-viewport internal-main">
   <header className="app-header internal-topbar"><button ref={menuButton} className="btn btn-ghost icon-btn mobile-menu-toggle" aria-label={mobile?'Fechar menu':'Abrir menu'} aria-expanded={mobile} aria-controls="app-navigation" onClick={()=>setMobile(value=>!value)}><Menu/></button><button className="btn btn-ghost icon-btn sidebar-collapse-toggle" onClick={()=>setCollapsed(value=>!value)} aria-label={collapsed?'Expandir menu lateral':'Recolher menu lateral'} aria-expanded={!collapsed} aria-controls="app-navigation">{collapsed?<PanelLeftOpen/>:<PanelLeftClose/>}</button><SphexPayLogo className="header-brand" adaptiveTheme/><div className="internal-topbar-context"><span>SPHEX WORKSPACE</span><strong>{title}</strong></div><div className="mobile-page-title">{title}</div><SearchInput/><div className="header-actions"><button className="btn btn-ghost icon-btn theme-toggle" onClick={()=>setTheme(theme==='light'?'dark':'light')} aria-label="Alternar tema">{theme==='light'?<Moon/>:<Sun/>}</button><NotificationBell/><ProfileMenu name={name} email={user?.email} onSignOut={logout}/></div></header>
