...
export function NotificationBell(){const [open,setOpen]=useState(false);const root=useRef<HTMLDivElement>(null);const notifications=useDemoStore(s=>s.notifications);const unread=notifications.filter(n=>!n.read).length;const latest=notifications[0]?.id;const previous=useRef(latest);const [pulse,setPulse]=useState(false)
