...
export function SalesNotificationToast({toast,onClose,onOpen}:{toast:VisibleToast;onClose:()=>void;onOpen:()=>void}){
 ...
 return <article className={`sales-notification-toast ${toast.leaving?'leaving':''}`} role="status" aria-live="polite" onClick={onOpen}><div className="sales-toast-logo"><SphexPayLogo/></div><div className="sales-toast-copy"><header><strong>{toast.title||notificationTitles[toast.type]}</strong><time dateTime={toast.createdAt}>{time}</time></header><p>{toast.body||formatCommission(toast.commission,toast.currency)}</p><small>{toast.status||'Atualização recebida'}</small></div><button aria-label="Fechar notificação" onClick={event=>{event.stopPropagation();onClose()}}><X/></button></article>
}
