...
export function MobileAppDock(){
 const location=useLocation(),navigate=useNavigate(),[more,setMore]=useState(false),[mobile,setMobile]=useState(()=>matchMedia('(max-width: 767px)').matches),opacity=useDemoStore(state=>state.preferences.mobileNavigation.opacity),index=mobileNavigationIndex(location.pathname),close=useCallback(()=>setMore(false),[])
