...
import { RefreshCcw,RotateCcw,Volume2 } from 'lucide-react'
...
export function SoundSettings(){
 const store=useDemoStore(),assistant=store.preferences.assistant
 ...
 const restore=()=>{updateAssistant({readAloud:true,volume:1,voice:'',voiceGender:'female',language:'pt-BR',speechRate:.96,pitch:1.04});setMessage('Configurações de voz restauradas.')}
 ...
 return <div className="sound-settings">
  ...
  <div className="sound-settings-grid">
   <section><h3>Voz da inteligência artificial</h3><Toggle label="Áudio da inteligência artificial" checked={sound.enabled} onChange={value=>{if(value)void activate();else sound.disable()}}/><Toggle label="Ler respostas em voz alta" checked={assistant.readAloud} onChange={readAloud=>updateAssistant({readAloud})}/><p className="text-xs muted mt-4">Sons de notificações comerciais são controlados exclusivamente pelo sistema operacional.</p></section>
   <section><h3>Voz</h3><Range label="Volume da voz" value={assistant.volume} onChange={volume=>updateAssistant({volume})}/><div className="voice-gender sound-gender">{([['female','Voz feminina'],['male','Voz masculina'],['auto','Automática']] as const).map(([value,label])=>{const profile=value==='female'?speech.pair.female:value==='male'?speech.pair.male:speech.pair.device;return <button disabled={!profile} className={assistant.voiceGender===value?'active':''} onClick={()=>profile&&updateAssistant({voiceGender:value,voice:profile.voiceURI})} key={value}>{label}</button>})}</div></section>
  </div>
  <div className="sound-actions"><button className="btn btn-primary" onClick={testVoice}><Volume2/> Testar voz</button><button className="btn" onClick={activate}><RefreshCcw/> Reativar áudio</button><button className="btn" onClick={restore}><RotateCcw/> Restaurar configurações</button></div>
