...
export function LiveSalesFeedContent({sales,displayCurrency,rates,limit=7,planning=false}:{sales:FinancialTransaction[];displayCurrency:Currency;rates:ExchangeRate[];limit?:number;planning?:boolean}){
 ...
 return <Card className="sales-card"><div className="sales-header"><div><span className="section-eyebrow"><i/> {demo?'ATIVIDADE RECENTE':planning?'TIMELINE DE CENÁRIO':'FLUXO DE VENDAS'}</span><h2>{planning?'Resultados projetados':'Vendas recentes'}</h2><p>{demo?'Acompanhe as atualizações mais recentes':planning?'Registros sintéticos restritos ao planejamento':'Transações persistidas no backend'}</p></div></div><div className="sales-list scrollbar">{rows.length?rows:<EmptyFinancialState/>}</div></Card>
}
