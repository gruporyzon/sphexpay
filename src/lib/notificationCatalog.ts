...
export function saleEventType(sale:Sale):CommerceNotificationType{
 ...
 if(sale.status==='Recusado')return'payment_refused'
 if(sale.status==='Pendente')return sale.method==='Pix'?'pix_generated':sale.method==='Boleto'?'boleto_generated':'sale_pending'
 if(sale.status==='Aprovado')return sale.method==='Cartão de crédito'?'credit_card_approved':sale.method==='Assinatura'?'subscription_approved':'sale_approved'
 return'sale_pending'
}
