...
export default function LiveSalesWorld(){
 ...
 const toggle=async()=>{await mode.toggle()}
 return <div className="live-world-page page-enter">
  <PageTitle title="Vendas ao Vivo" subtitle="Acompanhe transações globais em tempo real" action={<div className="live-world-head-actions"><span className="live-world-status"><i/> Ao vivo</span>{mode.active&&<span className="live-world-mode"><Radio/> Modo demo ativo</span>}{mode.allowed&&<button className={`live-world-toggle ${mode.active?'active':''}`} onClick={()=>void toggle()} aria-pressed={mode.active} disabled={mode.loadingPermission}>{mode.active?'Desativar modo demo':'Ativar modo demo'}</button>}</div>}/>
