...
export default function LiveSales({transactions=false}:{transactions?:boolean}){
 ...
 return <div className={transactions?'transactions-page internal-real-page page-enter':'live-sales-page internal-real-page page-enter'}>
  ...
  <Card className="transactions-table-card"><div className="transactions-toolbar internal-filter-bar"><div className="transactions-search"><SearchBox value={query} onChange={setQuery} placeholder="Comprador, produto ou identificador..."/></div><label className="transactions-filter internal-filter-select"><span>Status</span><select className="input internal-filter-value" aria-label="Filtrar por status" value={status} onChange={event=>setStatus(event.target.value)}><option value="all">Todos</option>{Object.entries(transactionStatusLabels).map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label></div>
