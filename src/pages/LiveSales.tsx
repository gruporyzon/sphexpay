...
const demoRates:ExchangeRate[]=Object.entries(demoExchangeRates).filter(([currency])=>currency!=='BRL').flatMap(([currency,rate])=>[{baseCurrency:currency as 'USD'|'EUR',quoteCurrency:'BRL',rate:1/rate,source:'taxa demonstrativa',observedAt:new Date().toISOString()},{baseCurrency:'BRL',quoteCurrency:currency as 'USD'|'EUR',rate,source:'taxa demonstrativa',observedAt:new Date().toISOString()}])
...
export default function LiveSales({transactions=false}:{transactions?:boolean}){
 ...
 return <div className="page-enter">
  <PageTitle title={transactions?'Transações':'Vendas'} subtitle={live.demo?'Registros demonstrativos locais. Ações financeiras reais estão desabilitadas.':transactions?'Movimentações financeiras persistidas no Supabase.':'Vendas reais confirmadas e atualizadas em tempo real.'} action={!live.demo?<button className="btn" onClick={()=>void live.refresh()} disabled={live.loading}><RefreshCcw className={live.loading?'spin':''}/> Atualizar</button>:undefined}/>
