...
export default function Dashboard(){
 ...
 const effectiveRates=useMemo<ExchangeRate[]>(()=>live.demo?Object.entries(demoExchangeRates).filter(([base])=>base!=='BRL').flatMap(([base,rate])=>[{baseCurrency:base as 'USD'|'EUR',quoteCurrency:'BRL' as const,rate:1/rate,source:'taxa demonstrativa',observedAt:new Date().toISOString()},{baseCurrency:'BRL' as const,quoteCurrency:base as 'USD'|'EUR',rate,source:'taxa demonstrativa',observedAt:new Date().toISOString()}]):rates,[live.demo,rates])
