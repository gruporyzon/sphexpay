...
export default function Dashboard(){
 ...
 return <div className="page-enter dashboard-page">
  ...
  <PageTitle title="Dashboard" subtitle={live.demo?'Acompanhe resultados, vendas e indicadores em tempo real.':'Resultados financeiros persistidos e planejamento administrativo isolado.'} action={<div className="dashboard-header-actions">{admin&&!live.demo?<button className="btn" onClick={()=>setMode(current=>current==='production'?'planning':'production')}>{planning?'Ver produção':'Editar planejamento'}</button>:null}{planning&&<DashboardScenarioEditor scenario={planner.scenario} rates={rates} onSave={planner.save} onSaveRates={saveRates}/>} {!planning&&!live.demo&&<RealtimeStatus iconOnly status={live.realtime} updatedAt={live.updatedAt} onRefresh={()=>void live.refresh()} loading={live.loading}/>}</div>}/>
  {adminAccess.error&&<p className="dashboard-admin-warning">{adminAccess.error}</p>}
  <Card className="dashboard-filter-bar"><div className="dashboard-filter-controls"><DashboardPeriodFilter period={period} onChange={setPeriod}/><DashboardCurrencySelector currency={currency} onChange={setCurrency}/></div>{(adminAccess.loading||admin&&!mobileLayout)&&<div className="dashboard-filter-actions">{adminAccess.loading?<span className="dashboard-admin-state">Validando acesso administrativo...</span>:<DashboardLayoutButton editor={layoutEditor}/>}</div>}</Card>
