...
export default function LandingPage(){
 ...
 useEffect(()=>{
  document.title='SphexPay — Gateway e plataforma financeira para negócios digitais'
 ...
 return <div className="landing" id="top">
  ...
  <main id="public-content">
   ...
   <section className="landing-hero" aria-labelledby="landing-title"><div className="hero-grid"/><div className="hero-orb orb-one"/><div className="hero-orb orb-two"/>
    <div className="landing-hero-copy"><span className="landing-kicker"><i/> Infraestrutura para negócios digitais</span><h1 id="landing-title">Venda, receba e escale com a <em>SphexPay</em></h1><p>Uma plataforma financeira para organizar pagamentos, vendas, clientes, produtos e resultados em uma visão centralizada.</p><div className="landing-hero-actions"><Link className="public-primary" to={accountDestination}>{user?'Acessar meu painel':'Criar minha conta'} <ArrowRight/></Link><a href="#plataforma">Conhecer a plataforma</a></div><ul className="hero-assurances" aria-label="Diferenciais de acesso"><li><Check/> Cadastro orientado</li><li><Check/> Visão em tempo real quando conectada</li><li><Check/> Acesso protegido</li></ul>
    </div><div className="landing-hero-visual"><AnimatedDashboard/></div><a className="hero-scroll" href="#plataforma"><span>Explore a plataforma</span><ChevronDown/></a>
   </section>
   ...
   <section className="live-public-section" id="vendas-ao-vivo"><div data-reveal><span>ACOMPANHAMENTO VISUAL</span><h2>Entenda o movimento da operação enquanto ele acontece.</h2><p>O módulo Vendas ao Vivo reúne mapa, feed, países e métricas. Na apresentação pública, a prévia é estática e não inicia consultas, timers ou o motor demonstrativo.</p><Link to={accountDestination} className="public-text-link">{user?'Abrir Vendas ao Vivo':'Conhecer a plataforma'} <ArrowRight/></Link></div><div className="live-public-visual" data-reveal aria-label="Prévia visual estática de Vendas ao Vivo"><Globe2/><div className="live-map-lines"><i/><i/><i/></div><div className="live-public-feed"><span><b>Atividade global</b><small>Eventos conectados aparecem aqui</small></span><span><b>Mapa de vendas</b><small>Distribuição visual por região</small></span><span><b>Visão consolidada</b><small>Métricas sem dados sensíveis</small></span></div></div></section>
