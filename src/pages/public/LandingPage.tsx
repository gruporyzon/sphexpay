...
export default function LandingPage(){
 ...
 return <div className="landing landing-v2" id="top">
  ...
  <main id="public-content">
   ...
   <section className="payment-section public-section--dark" id="solucoes"><div data-reveal><span>JORNADAS DE COBRANÇA</span><h2 className="public-heading--on-dark">Meios de pagamento organizados com transparência.</h2><p className="public-copy--on-dark">A interface contempla Pix, cartão, boleto, assinaturas e links. O processamento real e a disponibilidade de cada modalidade dependem de integrações oficiais ainda não conectadas neste ambiente.</p><div className="condition-card"><strong>Condições ajustadas à sua operação</strong><p>Conheça as condições disponíveis para o perfil e o volume do seu negócio quando os provedores oficiais estiverem integrados.</p><Link to={accountDestination}>{user?'Acessar painel':'Criar conta'} <ArrowRight/></Link></div></div><div className="payment-methods" data-reveal>{[[Zap,'Pix'],[CreditCard,'Cartão'],[Layers3,'Boleto'],[RefreshCcw,'Assinaturas'],[Link2,'Links']].map(([Icon,label])=><article key={label as string}><Icon/><span><b>{label as string}</b><small>Disponibilidade vinculada à integração oficial</small></span></article>)}</div></section>
   ...
   <section className="security-section public-section--dark" id="seguranca"><div className="security-visual" data-reveal><div><ShieldCheck/><i/><i/><i/></div><span>CAMADA DE PROTEÇÃO</span></div><div data-reveal><span>SEGURANÇA DESDE O ACESSO</span><h2 className="public-heading--on-dark">Uma experiência privada antes de mostrar qualquer dado.</h2><p className="public-copy--on-dark">Autenticação por Supabase Auth, sessão renovável, rotas privadas protegidas e isolamento de credenciais sensíveis fora da interface.</p><div className="security-list"><article><LockKeyhole/><span><b>Autenticação real</b><small>E-mail, senha e provedores sociais quando habilitados.</small></span></article><article><ShieldCheck/><span><b>Rotas protegidas</b><small>O painel só é renderizado depois da validação da sessão.</small></span></article><article><Globe2/><span><b>Conexão HTTPS</b><small>Produção servida pela Vercel com transporte protegido.</small></span></article></div></div></section>
   ...
   <PublicSecurityNote/>
   <PublicFaq/>
   <section className="landing-cta public-section--dark" data-reveal><Sparkles/><span>COMECE COM CLAREZA</span><h2 className="public-heading--on-dark">Seu próximo nível começa com uma operação melhor.</h2><p className="public-copy--on-dark">Centralize sua gestão, acompanhe seus resultados e construa uma operação preparada para crescer.</p><div><Link className="public-primary" to={accountDestination}>{user?'Acessar meu painel':'Criar minha conta'} <ArrowRight/></Link><Link to="/entrar">Entrar na plataforma</Link></div><small>Cadastre-se e acompanhe sua evolução na Competição SphexPay.</small></section>
  </main>
