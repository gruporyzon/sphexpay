...
export default function NotificationsPage(){
 ...
 return <div className="page-enter notifications-page">
  <PageTitle title="Notificações" subtitle="Comunicados administrativos, segurança e premiações. Eventos Demo são sempre identificados." action={<div className="flex flex-wrap gap-2"><button className="btn" onClick={readAll} disabled={!unread}><CheckCheck/> Marcar todas como lidas</button><button className="btn" onClick={()=>{if(confirm('Limpar todo o histórico de notificações?'))clear()}} disabled={!notifications.length}><Trash2/> Limpar histórico</button><button className="btn btn-primary" onClick={()=>navigate('/app/configuracoes?secao=Notificações')}><Settings2/> Configurar</button></div>}/>
  <div className="notification-summary"><span>{unread}</span><div><strong>Notificações não lidas</strong><p>Notificações demonstrativas permanecem somente nesta interface e não geram Push.</p></div></div>
