...
export default function NotificationsPage(){
 ...
 return <div className="page-enter notifications-page internal-real-page">
  ...
  <div className="notification-filters internal-segmented-control scrollbar" role="tablist" aria-label="Filtrar notificações">{filters.map(item=><button role="tab" aria-selected={filter===item} className={filter===item?'active internal-segmented-item':'internal-segmented-item'} key={item} onClick={()=>{setFilter(item);setVisible(10)}}>{item}</button>)}</div>
