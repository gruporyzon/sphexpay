...
export interface Product { id:string; name:string; description:string; price:number; currency?:'BRL'|'USD'|'EUR'; billing:'Única'|'Recorrente'; monthly?:number; annual?:number; active:boolean; sales:number; revenue:number; color:string }
...
export type CommerceNotificationType='sale_approved'|'sale_pending'|'pix_generated'|'pix_approved'|'pix_paid'|'credit_card_approved'|'credit_card_refused'|'boleto_generated'|'boleto_paid'|'subscription_approved'|'subscription_renewed'|'refund_done'|'chargeback_received'|'withdrawal_requested'|'withdrawal_sent'|'withdrawal_completed'|'payment_refused'
export interface NotificationMetadata { buyer?:string; product?:string; amount?:number; commission?:number|null; currency?:Sale['currency']; method?:PaymentMethod; status?:string; country?:string; eventType?:CommerceNotificationType; source?:'backend'|'local'|'manual'|'test'|'demo' }
...
export interface NotificationPreferences { internal:boolean; device:boolean; sales:boolean; withdrawals:boolean; subscriptions:boolean; security:boolean; achievements:boolean; sound:boolean; vibration:boolean; frequency:NotificationFrequency; groupSimilar:boolean; muteRepeated:boolean; priorityApproved:boolean; priorityPix:boolean; priorityCard:boolean; saleApproved?:boolean; pixGenerated?:boolean; cardApproved?:boolean; boletoEvents?:boolean; subscriptionEvents?:boolean; withdrawalEvents?:boolean; soundVolume:number; soundStyle:'signal'|'pulse'|'soft'; quietHours:boolean; quietFrom:string; quietTo:string; doNotDisturb:boolean; importantOnly:boolean }
...
export interface MobileNavigationPreferences { opacity:number }
export interface AppPreferences { notifications:NotificationPreferences; assistant:AssistantPreferences; sales:SalesPreferences; mobileNavigation:MobileNavigationPreferences }
