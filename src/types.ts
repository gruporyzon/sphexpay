...
export interface NotificationMetadata { buyer?:string; product?:string; amount?:number; currency?:Sale['currency']; method?:PaymentMethod; status?:string; country?:string; source?:'backend'|'local'|'manual'|'test' }
...
export interface NotificationPreferences { internal:boolean; device:boolean; sales:boolean; withdrawals:boolean; subscriptions:boolean; security:boolean; achievements:boolean; sound:boolean; vibration:boolean; frequency:NotificationFrequency; groupSimilar:boolean; muteRepeated:boolean; priorityApproved:boolean; priorityPix:boolean; priorityCard:boolean; soundVolume:number; soundStyle:'signal'|'pulse'|'soft'; quietHours:boolean; quietFrom:string; quietTo:string; doNotDisturb:boolean; importantOnly:boolean }
