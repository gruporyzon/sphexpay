...
describe('experiência mobile',()=>{
 ...
 it('refina somente no mobile o topo e os banners principais',()=>{
  const marker=globalCss.indexOf('/* Mobile gateway hero — final cascade layer; desktop and tablet stay untouched. */')
  const mobile=globalCss.slice(marker)
  ...
  expect(mobile).toContain('aspect-ratio:16/9')
  expect(mobile).toContain('height:clamp(250px,56.25vw,278px)')
  expect(mobile).toContain('object-fit:contain')
