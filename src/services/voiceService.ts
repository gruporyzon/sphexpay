...
const score=(voice:SpeechSynthesisVoice,language:string,gender:VoiceGender)=>{const name=voice.name.toLowerCase(),lang=voice.lang.toLowerCase(),target=language.toLowerCase(),base=target.split('-')[0];let value=lang===target?120:lang.startsWith(`${base}-br`)?100:lang.startsWith(base)?70:0;if(base==='pt'&&lang.startsWith('pt-pt'))value+=10;if(voice.localService)value+=12;if(/natural|premium|enhanced|neural/.test(name))value+=18;if(gender==='female'&&femaleHints.some(h=>name.includes(h)))value+=30;if(gender==='male'&&maleHints.some(h=>name.includes(h)))value+=30;return value}
