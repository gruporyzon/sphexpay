...
export type PushSendResult={ok:boolean;code?:string;reason?:string;message:string;sent?:number;failed?:number;expired?:number;duplicates?:number;results?:Array<{deviceId:string;status:string;code?:string}>;httpStatus?:number}
...
type PushPayload={eventId:string;type:string;title:string;body:string;route:string;tag?:string;createdAt?:string;currency?:string;commission?:number|null;notificationType?:string;targetDeviceId?:string;targetDeviceIds?:string[];target?:'all'}
