...
export type PushSendResult={ok:boolean;eventId?:string;code?:string;reason?:string;message:string;sent?:number;failed?:number;expired?:number;duplicates?:number;results?:Array<{deviceId:string;status:string;code?:string}>;httpStatus?:number}
...
export const pushSubscriptionService={
 ...
 async sendManual(input:{notificationType:string;title:string;body:string;route:string;icon:string;deviceIds:string[];currency:string}){const eventId=`manual-${crypto.randomUUID?.()||Date.now()}`;const result=await this.send({eventId,type:'manual_notification',notificationType:input.notificationType,title:input.title,body:input.body,route:input.route,icon:input.icon,tag:`sphexpay-manual-${input.notificationType}`,createdAt:new Date().toISOString(),currency:input.currency,metadata:{notificationType:input.notificationType,currency:input.currency},targetDeviceIds:input.deviceIds});return{...result,eventId:result.eventId||eventId}},
